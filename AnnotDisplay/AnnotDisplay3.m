function varargout = AnnotDisplay3(varargin)
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @AnnotDisplay3_OpeningFcn, ...
                   'gui_OutputFcn',  @AnnotDisplay3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function AnnotDisplay3_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = AnnotDisplay3_OutputFcn(hObject, eventdata, handles) 
...
if ~isfield(handles,'annotFilename')
    ...
    %whoops
%     whoopsIndx=find(handles.sylType==4);
%     handles.sylType(whoopsIndx) = 102;
% 
%     whoopsIndx=find(handles.sylType==6);
%     handles.sylType(whoopsIndx) = 102;


%Merge 104s
    mergeIndx = find(handles.sylType == 104);
    adjIndx = find(diff(mergeIndx) == 1);
    pntr = mergeIndx(adjIndx);
    
    %Reassign the merged syllable to 106 and delete the second half sylType
    handles.sylType(pntr) = 106;
    handles.sylType(pntr+1) = [];

    %Do the merge by deleting the first end poitn and the second start point
    handles.sylEnd(pntr) = [];
    handles.sylStart(pntr+1) = [];

    %Delete the extra key index
    handles.IndxKey(pntr+1) = [];
...
%     parfor i=1:length(uniq)
    ...
    %Save to file
    save(handles.annotFilename,'elements','keys', '-v7'); %Changed version for faster saving 12/30/14
    result_save = 'done'
...
output_sylclus = SylClus3(input_sylclus);
